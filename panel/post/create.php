...
if (
    ...
    $query = 'SELECT * FROM `categories` WHERE id=?'; //inja migam pro to category ha begard bebin aya hamchin category hast?
    $statement = $pdo->prepare($query);
    $statement->execute([$_POST['cat_id']]);
    $category = $statement->fetch();

    $allowedMimes = ['png', 'jpg', 'jpeg', 'gif'];  //pasvand haye mojaz
    $imageMime = pathInfo($_FILES['image']['name'], PATHINFO_EXTENSION); //pasvand imagei ke karbar upload karde

    if (!in_array($imageMime, $allowedMimes)) {  // agar pasvand file karbare dar list pasvand haye mojazam nabod redirectesh kon
        redirect('panel/post');
    }
                    ...
                    <form action="<?= url('panel/post/create.php') ?>" method="post" enctype="multipart/form-data">
                        ...
                        <section class="form-group">
                            ...
                            <select class="form-control" name="cat_id" id="cat_id">
                                ...
                                $statement = $pdo->prepare($query);
                                $statement->execute();
                                $categgories = $statement->fetchAll();
