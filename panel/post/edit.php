...
global $pdo;
// -------------------------check isset post id to edit------------------------------
if (!isset($_GET['post_id'])) {
    redirect('panel/post');
}
// -------------------------check exist post_id in data base-------------------------
...
if (
    ...
    // -----------------check inputs category is exist in data base ---------------
    $query = 'SELECT * FROM `categories` WHERE id=?';
    $statement = $pdo->prepare($query);
    $statement->execute([$_POST['cat_id']]);
    $category = $statement->fetch();
    // ---------------check if my user want to upload picture or not ---------------
    if (isset($_FILES['image']) && $_FILES['image']['name'] !== '') {
        ...
        // if exist this file in my image file delete it
                ...
                <section class="col-md-10 pt-3">
                    ...
                    <form action="<?= url('panel/post/edit.php?post_id=' . $_GET['post_id']) ?>" method="post"
                        ...
                        <section class="form-group">
                            ...
                            <div class=" mt-2">
                                <img src="<?= asset($post->image) ?>" alt="" width="180">
                            </div>
                        </section>
                        <section class="form-group">
                            ...
                            <select class="form-control" name="cat_id" id="cat_id">
                                ...
                                <option value="<?= $category->id ?>"
                                    <?php if ($category->id == $post->cat_id) echo 'selected'; ?>>
                                    <?= $category->name ?>
                                </option>
